...
export class UsersService {
  ...
  // get's admin settings
  public async getAdminList(): Promise<CommonResponseModel> {
    const admins = await this.userModel.findOne({ role: 'Admin' });
    if (admins) {
      return { response_code: HttpStatus.OK, response_data: admins };
    } else {
      return {
        response_code: HttpStatus.OK,
        response_data: 'Could not fetch admin settings',
  ...
  public async registerNewUser(
    ...
    if (
      ...
      return {
        response_code: HttpStatus.BAD_REQUEST,
        response_data: 'Role Should be Admin Or User',
      };
    }
  }
